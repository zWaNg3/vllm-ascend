...
This patch wraps ``NPUModelRunner._sync_batch_across_dp`` to inject a
...
if _INJECT_COUNT > 0:
    ...
    _original = vllm_ascend.worker.model_runner_v1.NPUModelRunner._sync_batch_across_dp
    _INJECT_RANK = int(os.environ.get("VLLM_FAULT_INJECT_RANK", "0"))
    _counter = 0

    def _patched_sync_batch_across_dp(self, *args, **kwargs):
        global _counter
    ...
    vllm_ascend.worker.model_runner_v1.NPUModelRunner._sync_batch_across_dp = (
        _patched_sync_batch_across_dp
    )
