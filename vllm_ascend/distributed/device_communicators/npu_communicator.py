...
class _NpuAll2AllManager:
    ...
    Ascend's MC2 kernel has no kernel-side all2all mask, so the FT mask lives
    in the worker-side ``ElasticInfoMask`` (elastic_info) state. This manager
    exposes the all2all-manager interface (``update_mask`` /
    ``query_active_mask`` / ``query_fault`` / ``clean_buffers``) so the sentinel
    can go through ``get_ep_all2all_manager``.
    """
    ...
    def clean_buffers(self) -> None:
        """Reset the ``query_fault`` baseline to the current mask.
        ...
        call. The fault-detection baseline is re-anchored to the CURRENT mask
        so that dead ranks that are already known after a scale-down are NOT
        reported as a fresh fault by ``query_fault`` (the base-class contract:
        compare against the mask at the last recovery point).
        """
        self._last_mask = self.query_active_mask()
